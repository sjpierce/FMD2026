...
handling missing data. It recommends taking a 3-step, structured approach to 
that task. It is grounded in a few principles that I think are applicable both 
to the observational research designs that were the focus of TARMOS and to other 
...
relevant covariates. A full specification of the required analyses will reveal
the set of variables that *must be available* to achieve your scientific
objectives. It is missing data on those *essential variables* that pose problems
for you; missing data on *other variables* may be irrelevant to your study.
...
Deciding how to handle missing data requires understanding both concepts from 
the missing data literature and various methods that *could* be applied. 
Some general options for the decision can certainly be identified during step 1
but my opinion is that the actual choice ought to be made in step 2. 
:::
...
to begin by examining whether your data are suitable for the planned analysis. 
There is a growing literature with guidelines about planning and conducting 
initial data analyses that accomplish this purpose. One key message from that 
literature is that the initial data analyses do *not* address the substantive 
research questions and hypotheses. They focus on assessing whether it is 
feasible to conduct the substantive analysis as originally planned or you will 
instead have to adapt your analysis plan to deal with unexpected features of the 
data.

Here, I want to emphasize that a thorough diagnosis of the missing data issues
present in your data should be part of your initial data analysis. Those results
give you the rest of the information you need to make good decisions about
handling missing data in your substantive analyses. If you are lucky, there
won't be any missing data at all and you can just implement your statistical
analysis plan as-is. However, the missing data diagnostics in your initial data
analyses may also show you that the planned analysis will not yield trustworthy
answers unless you modify it to handle the missing data sensibly.
...
plan. The initial data analyses should provide evidence for why some of those
...
::: {.notes}
The third and final step in TARMOS is reporting your analysis. You paper or
report should describe the missing data issues you detected in Step 2, how you
handled them, and why you selected that approach. Obviously, you should then
include and interpret the results of your analyses and any sensitivity analyses 
or robustness checks that you conducted.
...
until we thoroughly understand the nature and scope of the problem on a given
study. That means the next logical question is "how can we diagnose the missing
data issues for a given study?"

Next, I'm going to cover some additional concepts that pertain to describing the
amounts, patterns, and mechanisms of missing data. It helps to start by thinking 
about data structures that affect how we organize and use data. 
:::
...
measures data. The long univariate format keeps all data about a given variable
in a single column, so each time point must become a distinct set of rows. Each
person ends up with one row per time point.
::: 
...
have any missing data and are used to uniquely identify a row of das belo nging
to a specific person at a specific time point. So we end up with a total of 6
cases and 5 variables, for a total of 30 possible data values, of which 18 are
the contents of the original data cube.
:::
...
:::{.notes}
The wide multivariate format instead keeps all data about a single case in one 
row, with each time point represented by a distinct set of variables. This table 
shows the same data you saw on the last slide, rearranged into wide format. 

So, now we have one row per unique ID, but we need six variables to represent 
X1, X2, and X3 at the two time points. I added a time prefix to each variable 
name, and grouped the variables by time point. The column grouping labels above
the variable names just clarify that structure. We now have a total of 21 
values, of which 18 are the contents of the original cube. 
:::
