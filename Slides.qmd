...
:::{.notes}
The 3-dimensional data cube concept is useful, but actual data files are often
constrined to 2-dimensional structure with rows for people (cases) and columns
for variables. We have to slice and rearrange the box to handle the time
dimension sensibly. There are two common formats for representing longitudinal
or repeated measures data. The long univariate format keeps all data about a
given variable in a single column, so each time point must become a distinct set
of rows. Each person ends up with one row per time point.

In this example, we have three variables X1, X2, and X3, each measured at two 
time points, for three people. In addition to the person-level ID variable, we 
need an explicit time variable so we can tell which row for each person 
...
So, now we have one row per unique ID, but we now need six variables to 
...
labels above the variable names just clarify that structure. 
:::
