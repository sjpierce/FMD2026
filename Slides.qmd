...
kind of missing data you have or identify why they are missing, but the heart of
the problem is that not having all the desired data means you have to decide how 
to adapt your plans for completing the study. 
...
events, data collection for one of my projects was disrupted by a an unusually 
...
accidentally skip entering one or more items that were actually recorded when
entering data from paper forms into electronic files.
...
::: {.notes}
The relevance of those ASA guidelines to the handling of missing data become 
...
given variable in a single column, so it each time point must become a distinct
set of rows. Each person ends up with one row per time point.

Here we need an explicit time variable so we can tell which row for each person
corresponds to a specific time point. We have collected two variables X1 and X1 
at each time point, and of course we have an ID variable to show which person 
was observed. 
:::
...
row, with each time point represented by a distinct set of variables. Here, we 
have two variables for T1_X1 for X1 at time 1, and another column T2_X1 for the 
same variable measured at a later time 2. I've added column grouping labels 
above the variable names to clarify the structure. 
:::
