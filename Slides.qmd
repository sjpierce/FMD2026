...
However, there are some post-data collection things that can happen too. For
example, paper files might get lost or damaged after data collection before
anyone even has a chance to enter them into an electronic data file. Another 
possibility is that your staff entering data from paper forms into electronic
files may accidentally skip entering one or more items that were actually
recorded. That can introduce missing data at that stage.
...
drive might fail. Finally, sometimes we make errors in during data analysis 
...
Association's ethical guidelines include a section on professional integrity 
that says an ethical statistical practitioner 
...
::: {.incremental}
* Analyzing data with missing values requires making assumptions, some of which 
  cannot be empirically tested. 
* Rigor demands using methods that can plausibly yield unbiased estimates. Poor
  assumptions lead to bias. 
* Transparency requires reporting the extent of missing data, how we handled 
  them, & defending our assumptions. 
  
:::

::: {.notes}
Craig Enders and Amanda Gottschall wrote a paper that articulates the 
relationship between how we handle missing data and the ethical quality of a 
study. They start from the premise that the ethical quality of an analysis 
depends on the credibility of its assumptions.

▶ They go on to point out that analyzing data with missing values requires 
making assumptions, some of which cannot be empirically tested. We have to pay
attention to which assumptions are reasonable and defensible. That will vary
across studies. 
...
Conducting analyses built on poor assumptions about missing data leads to biased 
and misleading results. And that would be unethical.

▶ It follows then that transparency requires reporting things like the extent 
of missing data, how we handled them, and defending our assumptions. 
:::
